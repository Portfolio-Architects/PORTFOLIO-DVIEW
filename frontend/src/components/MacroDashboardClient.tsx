...
export default function MacroDashboardClient({
  ...
  const popularAptItems = useMemo(() => {
    if (!sheetApartments || !favoriteCounts || !txSummaryData) return [];
    ...
    const items = apts.map((apt) => {
      ...
      const summary = txKey ? txSummaryData[txKey] : undefined;
  ...
  }, [sheetApartments, favoriteCounts, txSummaryData, nameMapping, publicRentalSet]);

  const [tickerIndex, setTickerIndex] = useState(0);
  const [isTickerHovered, setIsTickerHovered] = useState(false);

  useEffect(() => {
    if (isTickerHovered || popularAptItems.length <= 1) return;
    const interval = setInterval(() => {
      setTickerIndex((prev) => (prev + 1) % popularAptItems.length);
    }, 3000);
    return () => clearInterval(interval);
  }, [isTickerHovered, popularAptItems.length]);
  ...
  return (
    <div className="w-full flex flex-col bg-surface relative">
      ...
      <div className="flex flex-col px-4 sm:px-6 md:px-10 lg:px-16 pt-3 md:pt-5 pb-0 md:pb-12 lg:pb-16 w-full">
        ...
        {/* 실시간 인기 단지 롤링 티커 */}
        {popularAptItems.length > 0 && (
          <div 
            className="relative mb-5 flex items-center justify-between bg-white dark:bg-zinc-900 border border-border/80 rounded-[14px] px-4 py-2.5 shadow-sm hover:shadow-md transition-all duration-300 z-30 cursor-pointer"
            onMouseEnter={() => setIsTickerHovered(true)}
            onMouseLeave={() => setIsTickerHovered(false)}
          >
            {/* Ticker Content */}
            <div 
              className="flex items-center gap-2.5 overflow-hidden flex-1 select-none" 
              onClick={() => onSelectApt && onSelectApt(popularAptItems[tickerIndex].name)}
            >
              <span className="flex items-center justify-center bg-teal-500/10 text-teal-600 dark:text-teal-400 font-extrabold text-[12px] px-2.5 py-0.5 rounded-[8px] shrink-0 gap-1">
                <span className="animate-pulse">🔥</span> 실시간 인기 {tickerIndex + 1}위
              </span>
              <div className="relative h-[24px] overflow-hidden flex-1">
                <div 
                  className="flex flex-col transition-transform duration-500 ease-in-out"
                  style={{ transform: `translateY(-${tickerIndex * 24}px)` }}
                >
                  {popularAptItems.map((item, idx) => (
                    <div key={item.name} className="h-[24px] flex items-center justify-between w-full">
                      <div className="flex items-center gap-1.5 min-w-0 flex-1">
                        ...
                        <span className="text-[13.5px] font-extrabold text-primary truncate hover:text-teal-500 transition-colors">
                          {item.name}
                        </span>
                      </div>
                      <div className="flex items-center gap-3 shrink-0 ml-4 text-[12.5px]">
                        {item.latestPrice && (
                          <span className="text-secondary font-bold hidden sm:inline">
            ...
            <div className="flex items-center gap-1 ml-2 text-tertiary shrink-0">
              <span className="text-[11px] font-bold hidden sm:inline text-secondary/80">전체보기</span>
              <ChevronDown size={14} className={`transition-transform duration-300 ${isTickerHovered ? 'rotate-180 text-primary' : ''}`} />
            </div>

            {/* Hover Popover Dropdown */}
            <div className={`absolute left-0 right-0 top-full mt-1.5 bg-white dark:bg-zinc-950 border border-border rounded-2xl shadow-xl p-3.5 z-50 flex flex-col gap-1 transition-all duration-200 ${isTickerHovered ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-1 pointer-events-none'}`}>
              <div className="flex items-center justify-between px-2 py-1.5 border-b border-border/60 mb-2">
                ...
                <span className="text-[11px] text-tertiary font-bold">D-VIEW 실시간 관심도 기준</span>
              </div>
              {popularAptItems.map((item, idx) => (
                <div 
                  key={item.name}
                  onClick={(e) => {
                    e.stopPropagation();
                    onSelectApt && onSelectApt(item.name);
