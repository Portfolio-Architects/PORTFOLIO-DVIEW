...
const TechnoValleyDashboard = React.memo(function TechnoValleyDashboard() {
  ...
  const handleSetMetricModeVacancy = useCallback(() => setMetricMode('vacancy'), []);
  const handleSetMetricModeRent = useCallback(() => setMetricMode('rent'), []);
  const handleTimeframeChange = useCallback((tf: '3Y' | '6M' | 'YTD' | '1Y' | 'ALL') => setTimeframe(tf), []);

  const handleToggleVisibleBuilding = useCallback((buildingId: string) => {
    setVisibleBuildings(prev => {
      if (prev.includes(buildingId)) {
        if (prev.length > 1) {
          return prev.filter(x => x !== buildingId);
        }
        return prev;
      }
  ...
  const handleToggleSelectedBuilding = useCallback((buildingId: string) => {
  ...
  const handleResetActiveCategory = useCallback(() => {
    setActiveCategory(null);
  }, []);
  ...
  const processedSectors = useMemo(() => {
    ...
    return donutData.map((sector: any) => {
      const companies = sector.companies || [];
      const filteredCompanies = query
        ? companies.filter((co: string) => co.toLowerCase().includes(query))
        : companies;
      return {
        ...sector,
        companies,
  ...
  }, [donutData, deferredSearchQuery]);

  const totalMatchedCount = useMemo(() => {
    return processedSectors.reduce((acc: number, sector: any) => acc + sector.filteredCompanies.length, 0);
  }, [processedSectors]);
  ...
  return (
    <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 mb-5 items-stretch">
      ...
      <div className="lg:col-span-6 flex flex-col gap-6 lg:h-[586px]">
        <ChartErrorBoundary fallbackText="업종별 분포 차트를 불러올 수 없습니다.">
                        ...
                        {donutData.map((entry: any, index: number) => {
                          ...
                          return (
                            <Cell 
                               ...
                               onClick={() => handleSelectCategory(entry.name)}
                            />
                          );
                        })}
                      </Pie>
        ...
        {/* 2x2 KPI Cards Grid */}
        <ErrorBoundary name="테크노밸리 핵심 지표">
      ...
      <div className="lg:col-span-6 bg-surface border border-border/80 p-4 sm:p-6 rounded-[20px] sm:rounded-[24px] shadow-sm flex flex-col justify-between lg:h-[566px] min-h-[460px]">
        <ChartErrorBoundary fallbackText="공실률 및 임대료 추이 차트를 불러올 수 없습니다.">
          {/* Chart Header */}
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 pb-3 border-b border-border/40 w-full min-w-0 overflow-hidden">
            ...
            <div className="flex flex-wrap items-center gap-2 self-stretch sm:self-auto justify-between sm:justify-end w-full sm:w-auto">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex bg-body/80 p-0.5 border border-border/40 rounded-lg shadow-inner">
                  <button
                    onClick={handleSetMetricModeVacancy}
                  ...
                  <button
                    onClick={handleSetMetricModeRent}
                    className={`px-2 py-0.5 sm:py-1 text-[9.5px] sm:text-[10px] font-extrabold rounded-md transition-all cursor-pointer ${
                      metricMode === 'rent' 
                        ? 'bg-surface text-primary shadow-sm' 
                        : 'text-tertiary hover:text-secondary'
          ...
          <div className="flex flex-wrap gap-2 mb-4 border-b border-border/40 pb-4">
            {AVAILABLE_BUILDINGS.map(b => {
              ...
              return (
                <button
                  key={b.id}
                  onClick={() => handleToggleVisibleBuilding(b.id)}
      ...
      <div className="lg:col-span-12">
        <ErrorBoundary name="업종별 기업 리스트">
          <div className="bg-surface border border-border/80 p-4 sm:p-6 rounded-[20px] sm:rounded-[24px] shadow-sm flex flex-col gap-4">
            ...
            {deferredSearchQuery && totalMatchedCount === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {processedSectors.map((sector: any) => {
                  const companies = sector.companies || [];
                  const filteredCompanies = sector.filteredCompanies || [];

                  if (deferredSearchQuery && filteredCompanies.length === 0) {
                    return null;
                  }

                  const isExpanded = deferredSearchQuery ? true : !!expandedSectors[sector.name];
                  const visibleCount = visibleCounts[sector.name] || 12;
                  const visibleCompanies = deferredSearchQuery ? filteredCompanies : filteredCompanies.slice(0, visibleCount);
                  const hasMore = !deferredSearchQuery && filteredCompanies.length > visibleCount;
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[11px] font-extrabold text-secondary bg-body dark:bg-zinc-800 px-2 py-0.5 rounded-full">
                          {deferredSearchQuery ? `${filteredCompanies.length}개 매칭` : `${companies.length}개 기업`}
                        </span>
                      </div>
                      {isExpanded ? (
                      ...
                      <div className="p-4 bg-surface/50 border-t border-border/40 animate-in fade-in slide-in-from-top-1 duration-200">
                        {companies.length === 0 ? (
                          ...
                          <>
                            ...
                            {/* Pagination Buttons */}
                            {(hasMore || (!deferredSearchQuery && visibleCount > 12)) && (
                              <div className="flex items-center justify-center gap-2 mt-4 pt-3 border-t border-border/30">
                                ...
                                {!deferredSearchQuery && visibleCount > 12 && (
                                  <button
                ...
                <div className="flex flex-wrap gap-2 p-3 bg-body/80 border border-border/40 rounded-2xl">
                  {AVAILABLE_BUILDINGS.map(b => {
                    ...
                    return (
                      <button
                        key={b.id}
                        onClick={() => handleToggleSelectedBuilding(b.id)}
...
export default TechnoValleyDashboard;
