    ...
    <div className="w-full bg-surface rounded-2xl border border-border p-5 flex flex-col gap-4 shadow-sm min-h-[300px]">
      <div className="flex justify-between items-center border-b border-border/50 pb-3.5">
        ...
        <button 
          ...
          className="text-[11.5px] font-bold text-secondary hover:text-hs-orange transition-colors bg-transparent border-none cursor-pointer"
        >
          라운지 전체보기 ➔
        </button>
      </div>
        ...
        {(!postsData?.posts || postsData.posts.length === 0) ? (
          ...
          postsData.posts.slice(0, 4).map((post: LoungeTalkPostItem) => (
            <button 
              ...
              className="flex justify-between items-center p-3 hover:bg-body/50 dark:hover:bg-zinc-950/20 border border-transparent hover:border-border/30 rounded-xl transition-all cursor-pointer group active:scale-[0.995] w-full text-left bg-transparent outline-none focus-visible:ring-2 focus-visible:ring-hs-orange/50"
            >
              <div className="flex flex-col gap-1 min-w-0 mr-3">
                <div className="flex items-center gap-1.5">
                  <span className="text-[9.5px] font-bold text-hs-orange bg-hs-orange/10 dark:bg-hs-orange/20 px-1.5 py-0.5 rounded shrink-0">
                    {post.category || '기타'}
                  </span>
                  <span className="text-[12.5px] font-bold text-primary truncate group-hover:text-hs-orange transition-colors">
                    {post.title}
                  </span>
                </div>
