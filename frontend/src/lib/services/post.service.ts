...
export class PostService {
  constructor(
    private postRepo: typeof PostRepo = PostRepo,
    private userRepo: typeof UserRepo = UserRepo,
    private apartmentRepo: typeof ApartmentRepo = ApartmentRepo,
  ...
  async createPost(
    ...
    try {
      ...
      const postId = await this.postRepo.createPost({
      ...
      await this.syncManagerPostToScoutingReport(
  ...
  async syncManagerPostToScoutingReport(
    title: string,
    content: string,
    category: string,
    ...
    try {
      ...
      // Resolve apartments
      const apartments = validated.data.providedApartments || await this.apartmentRepo.fetchApartmentNames();
      ...
      if (scoredApts.length > 0) {
        ...
        await this.reportRepo.updatePremiumContentByApartment(targetAptName, cleanContent);
      } else {
...
export async function createPost(
  ...
  return postService.createPost(title, content, category, authorUid, imageFile, authorEmail, customNickname);
}
...
export async function syncManagerPostToScoutingReport(
  ...
  return postService.syncManagerPostToScoutingReport(title, content, category, authorEmail, providedApartments);
}
