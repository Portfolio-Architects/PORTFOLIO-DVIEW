...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const navLinks = [
    ...
    { href: '/admin/reports', label: '애널리틱스 리포트', icon: BarChart2, section: 'Reports' },
    { href: '/admin/engineering', label: '엔지니어링 리포트', icon: FileText, section: 'Reports' },
        ...
        {navLinks.map((link) => (
          <Link key={link.href} href={link.href}
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-[14px] font-semibold transition-all ${
              isActive(link.href) ? 'bg-[#c44d00] text-surface shadow-lg shadow-[#ea6100]/20' : 'text-secondary hover:bg-body'
            }`}>
            <link.icon size={18} />
            {link.label}
          </Link>
