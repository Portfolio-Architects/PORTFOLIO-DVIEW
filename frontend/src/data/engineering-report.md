...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-05-31 | **모바일 아파트 시세 추이 바텀 시트 겹침 현상 핫픽스 (Fix Mobile Bottom Sheet Z-Index Stacking & Padding Overlap)** | 모바일 화면에서 일자별 신고가 단지 카드를 터치할 때 뜨는 바텀 시트 모달이 하단 내비게이션 바(스티키 바텀 바, `MobileDock`)와 겹쳐서 컨텐츠가 잘리거나 어두운 Backdrop Overlay 뒤로 하단바가 툭 튀어나오는 Stacking Context 결함을 해결했습니다. 1) [MacroDashboardClient.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/MacroDashboardClient.tsx)의 바텀 시트 HTML 렌더링에 React Portal(`createPortal`)을 이식하여 모달 DOM을 `document.body` 바로 아래로 렌더링하도록 래핑했습니다. 이로써 `z-[9999]`의 절대 우선순위가 복구되어 모바일 하단바(`z-[999]`)를 정상적으로 덮습니다. 2) 바텀 시트 안쪽 스크롤 영역의 패딩을 기존 `pb-16`에서 `pb-24`로 확장 적용하여, 기기 하단 Safe Area 및 모바일 화면 바닥과 아코디언 테이블 사이의 충분한 가독 여백을 확보했습니다. 3) `npx tsc --noEmit` 컴파일 무결성 검증을 마쳤습니다. |
