...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-08-08 | **프로덕션 최근 실거래가 미노출 버그 수정 및 API 정적 폴백 이식 (Production Recent Transactions Display Safeguard & API Fallback Fix - Phase 914)** | 1) `nameMapping`이 빈 객체(`{}`)로 설정되어 `filteredRecentTransactions` 필터링 과정에서 17,000여 건의 모든 실거래 데이터가 빈 배열(`[]`)로 삭제되던 [DashboardClient.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/DashboardClient.tsx) 가드 조건문(`Object.keys(nameMapping).length === 0`)을 보완했습니다. 2) [/api/dashboard-init/route.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/app/api/dashboard-init/route.ts) API 라우트에 DB/Redis 응답 타임아웃 발생 시에도 `public/data/apartments-by-dong.json` 파일의 정적 메타데이터를 폴백으로 반환하는 스태틱 주입 로직을 추가했습니다. 3) `npm run build` 정적 컴파일 및 Next.js 프로덕션 빌드 무결성 검증을 100% 통과 완료했습니다. |
| 2026-08-01 | **모바일 아파트 카드 2행 수직 레이아웃 개편 및 가독성 최적화 (Mobile Apt Card 2-Row Vertical Layout Refactoring - Phase 913)** | 1) 모바일 뷰포트(< 480px) 환경에서 아파트 카드 내 아파트 이름 조기 생략(...) 및 신고가 뱃지 겹침 현상을 해결하기 위해 2행 수직 레이아웃 개편을 수행했습니다. 2) 1행에 `[신고가 뱃지]` + `[동 / 평형 / 층수]` 태그 정보, 2행에 `[아파트 Full Name]` (가로 폭 100% 활용)을 전용 배치하여 모바일에서도 아파트명이 말줄임 없이 시원하게 노출되도록 튜닝했습니다. 3) 우측 실거래 가격, 변동폭 및 [상세] 버튼의 수직 중앙 정렬 및 패딩을 최적화하고 [MacroDashboardClient.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/MacroDashboardClient.tsx) 등 관련 피드 UI 규격을 일원화했습니다. 4) 독립 승인 감사(Victory Audit), TypeScript 정적 타입 검사(`npx tsc --noEmit`), unit test 352개 전수 통과 및 Next.js 프로덕션 빌드(`npm run build`) 무결성 패스를 확인했습니다. |
