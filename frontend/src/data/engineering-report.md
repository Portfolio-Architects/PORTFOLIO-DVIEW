...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-05-31 | **동탄 로컬 캘린더 데스크톱(PC) 뷰 가로 스크롤 활성화 및 탐색 편의성 개선 (Enable Horizontal Scrollbar for Local Calendar in Desktop View)** | 로컬 캘린더 컴포넌트가 모바일 위주 터치 캐러셀 컨셉으로 설계되어, 가로 스크롤바와 관련된 CSS 숨김 속성 및 앱 전역 가로 스크롤바 숨김 정책에 의해 PC 마우스 환경에서 탐색하기 어렵던 심각한 사용성 제약을 해결했습니다. 1) [globals.css](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/app/globals.css#L261)에 데스크톱 뷰 전용의 얇고 심플한 가로 스크롤바 스타일(`custom-h-scrollbar`)을 선언하고, 글로벌 숨김 정책을 오버라이딩(`!important` 처리)할 수 있도록 설계했습니다. 2) [LocalCalendar.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/lounge/LocalCalendar.tsx#L93)에 데스크톱 브레이크포인트(`md:custom-h-scrollbar`)를 바인딩하여, 모바일 환경에서는 기존의 터치 슬라이드(숨김 스크롤)를 유지하되 PC 해상도 이상에서는 얇고 미려한 스크롤바가 노출되도록 개선했습니다. 3) 데스크톱 뷰 한정 하단 내부 여백(`md:pb-4`)을 확장하여 스크롤바가 콘텐츠 레이아웃을 침범하지 않도록 여유 영역을 확보했으며, 린트/컴파일 검증(`npm run audit` PASS)을 완료했습니다. |
