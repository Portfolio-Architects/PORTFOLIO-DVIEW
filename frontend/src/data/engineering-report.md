...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-05-31 | **모바일 바텀 시트 하단바 노출, 데스크톱 매크로 기본화 및 가격 기준 모니터링 개편 (Expose Mobile Sticky Bar, Unify Default Macro Trend & Implement Price-based Market Summary)** | 모바일과 데스크톱 전반의 가격 추이 렌더링 및 시장 요약 정보 구조를 사용자 친화적인 스펙으로 고도화했습니다. 1) [MobileDock.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/pwa/MobileDock.tsx)의 z-index를 `z-[10000]`으로 격상하여 Portal로 뜬 바텀 시트(`z-[9999]`) 위로 항상 드러나게 하고 스크롤 패딩을 `pb-36`으로 확보했습니다. 2) [MacroDashboardClient.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/MacroDashboardClient.tsx)의 초기 마운트 시 신고가 첫 번째 단지를 강제 선택하여 그래프를 덮어씌우던 `useEffect`를 제거했습니다. 이를 통해 데스크톱 뷰포트에서도 초기에는 "동탄 아파트 대표 가격 변화 추이"(매크로 그래프)를 우선 노출하고, 단지 클릭 시에만 개별 단지 가격 추이로 매끄럽게 전환되도록 변경했습니다. 3) 시장 요약 종합 모니터링 영역의 지표를 기존 거래량 기준(실거래량, 거래추세)에서 최신 매크로 포인트 데이터를 활용한 **가격 기준("시장 평균가", "전세 평균가")**으로 개편하여 부동산 자산 가치 모니터링 본연의 데이터 전달력을 극대화했습니다. 4) `npx tsc --noEmit` 컴파일 무결성 검증을 완료했습니다. |
