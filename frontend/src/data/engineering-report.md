...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-05-30 | **동탄구 소식 탭 명칭 리팩토링 및 오염된 화성시 공지 수집 필터 오류 수정 (Rename notices tab to Dongtan Notice & Fix Hwaseong Notice Filter Leak)** | 기존의 "동탄구청 소식" 카테고리를 "동탄구 소식"으로 일괄 개명했습니다. 또한 화성시 고시공고 수집 시 동탄 무관한 공지(예: 오산 특별공급 등)까지 DB에 적재되던 수집 버그를 해결하기 위해, 크론 수집기(`sync-local-notices/route.ts`)에서 동탄 키워드가 매칭된 공지만 Firestore에 선택적으로 저장하도록 필터링을 강화했습니다. 추가적으로 `/api/local-notices` API의 기본 필터값(`filterDongtan = true`)을 활성화하여 프론트엔드 연동 시 노이즈 없는 깨끗한 동탄 관련 소식 리스트만 노출되도록 보장했습니다. |
