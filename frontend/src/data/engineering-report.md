...
| 분류 | 기술 | 비고 |
|:---|:---|:---|
| 2026-06-01 | **프로덕션 환경 Firebase 로그인 불능 오류 해결을 위한 CSP(Content Security Policy) connect-src 도메인 예외 추가 및 보안 무결성 확보 (Resolve Production Login Failure by Enhancing CSP connect-src and Hardening AppCheck reCAPTCHA)** | 1) PWA Service Worker가 백그라운드에서 Google reCAPTCHA 및 AdSense 관련 정적 스크립트를 로드할 때 브라우저의 CSP `connect-src` 정책에 가로막혀 네트워크 연결이 차단되고 이로 인해 로그인 기능이 응답하지 않던 오류를 해결했습니다. 2) [proxy.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/proxy.ts) 미들웨어의 `connect-src` 지시문에 `https://www.gstatic.com`, `https://*.gstatic.com`, `https://*.adtrafficquality.google`, `https://*.firebaseapp.com` 도메인을 추가하여 reCAPTCHA Enterprise, Firebase Auth, Google AdSense 소다 구성 설정 네트워크 요청을 안전하게 허용했습니다. 3) 정적 분석(`npm run audit` PASS)을 수행하여 TypeScript 컴파일 상태, ESLint 코드 위생 및 파이어스토어 예상 비용(₩4 KRW)에 대한 모든 자체 검증을 100% 통과했습니다. |
| 2026-06-01 | **모바일 뷰 실거래가 무한 스크롤 비활성화 및 수동 '더보기' 버튼 복원 (Restore Manual Show More for Mobile & Disable Automatic Infinite Scroll)** | 1) 모바일 뷰(화면 너비 768px 미만)에서 스크롤 영역 높이 제약이 풀려있어 `useInView` 훅에 의해 전체 실거래 내역이 자동 로드되어 버리던 문제를 방지하고자, 모바일 환경에서는 자동 무한 스크롤이 트리거되지 않도록 `isMobile` 감지 로직을 추가했습니다. 2) 실거래 목록 하단의 spinner 영역 역시 모바일 뷰에서는 비활성화하여 자동 뷰포트 감지로 인한 불필요한 데이터 추가 렌더링을 완전히 차단했습니다. 3) 모바일 뷰 전용 수동 30건 단위 **'더보기' 버튼**을 테이블 하단에 세련된 토스 스타일(`bg-[#f2f4f6] text-[#4e5968]`)로 복원하여, 사용자가 원하는 경우에만 데이터 목록을 점진적으로 확장 탐색할 수 있도록 UX를 개편했습니다. 4) Next.js 빌드 및 하이드레이션 불일치(Hydration mismatch) 방지를 위해 `mounted` 상태 추적 설계를 적용했습니다. |
