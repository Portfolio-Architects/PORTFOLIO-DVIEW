/**
 * @module usePreloadApartmentTx
 * @description Preload helper and hook for apartment transactional data caching via SWR / apiClient.
...
export function getApartmentFileKey(
  ...
  const rawTxKey =
    overrideKey ||
    (rawApt as { txKey?: string })?.txKey ||
    findTxKey(apartmentName, txSummaryData, nameMapping, false, dong);
...
export function preloadApartmentTxData(fileKey: string): void {
  ...
  try {
    ...
    apiClient.get(recentUrl).catch(() => {});
  }

  try {
    preload(fullUrl, fetcher);
  } catch {
    apiClient.get(fullUrl).catch(() => {});
  }
}
...
export function usePreloadApartmentTx(
  ...
  const preloadApartmentTx = useCallback(
    (apartmentName: string, dong: string) => {
