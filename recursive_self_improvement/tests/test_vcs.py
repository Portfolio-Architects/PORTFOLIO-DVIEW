...
class TestCustomVCS(unittest.TestCase):
    ...
    def test_dual_snapshot_save(self):
        ...
        test_snap = os.path.join(self.test_dir, "target_test_module.v1.py")
    ...
    def test_rollback_missing_version_graceful_fallback(self):
        """
        Verifies edge case fix: rollback(version_idx) gracefully handles early limit aborts
        where snapshot files for version_idx do not exist yet.
        """
        # Save baseline version 0
        self.vcs.save_version(0, "baseline_code_v0", "baseline_test_v0")
        
        # Attempt to rollback to version 999 which does NOT exist in history
        self.assertFalse(self.vcs.has_version(999))
        
        # Should not raise FileNotFoundError, should fallback to baseline v0
        try:
            restored = self.vcs.rollback(999)
            self.assertEqual(restored, "baseline_code_v0")
        except FileNotFoundError:
            self.fail("rollback raised FileNotFoundError on missing version snapshot")
