...
class SelfImprovementEngine:
    ...
    def _finalize_and_generate_report(self) -> None:
        ...
        try:
            ...
            log_path = os.path.join(self.history_dir, "execution_log.json")
            reporter = ReportGenerator(log_path, self.history_dir, report_output)
            report_content = reporter.generate_markdown_report()
            timestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
            print(f"[{timestamp}] [REPORT] Improvement audit report generated at {report_output}.")
        except Exception as e:
    ...
    def evaluate_performance_degradation(self, candidate_metrics: BenchmarkMetrics) -> tuple[Optional[str], list[str]]:
        ...
        if base.execution_time_sec > 0 and candidate_metrics.execution_time_sec > base.execution_time_sec * (1.0 + lat_deg) and (candidate_metrics.execution_time_sec - base.execution_time_sec > 0.05):
        ...
        # 4. Memory degradation (1.0 MB noise floor filter to prevent sub-megabyte OS process memory noise false positives)
        is_mem_degraded = False
        if base.peak_memory_mb > 0 and candidate_metrics.peak_memory_mb > base.peak_memory_mb * (1.0 + mem_deg) and (candidate_metrics.peak_memory_mb - base.peak_memory_mb > 1.0):
    ...
    def run(self) -> bool:
        ...
        self.log_event("LOOP_START", "Self-improvement loop started.")
        ...
        try:
            ...
            self._finalize_and_generate_report()
            return False
        ...
        while True:
            ...
            while True:
                ...
                try:
                    ...
                    self.log_event("CANDIDATE_SIMULATED", f"Candidate code simulated for iteration {iteration}.", {
                        "iteration": iteration,
                        "strategy_rationale": getattr(self.simulator, "last_rationale", f"Optimization strategy applied for iteration {iteration}"),
                        "code_length": len(improved_code) if improved_code else 0
                    })
                    if self.perturbation_feedback or self.error_feedback:
                        self.log_event("STRATEGY_FEEDBACK", f"Strategy feedback provided for iteration {iteration}.", {
                            "iteration": iteration,
                            "perturbation_feedback": self.perturbation_feedback,
                            "error_feedback": self.error_feedback
                        })
            ...
            if code_hash in self.recent_hashes:
                ...
                self.log_event("STRATEGY_FEEDBACK", f"Stuck recovery strategy feedback updated for iteration {iteration}.", {
                    "iteration": iteration,
                    "perturbation_feedback": self.perturbation_feedback
                })
            ...
            self.log_event("AST_PRE_VALIDATE", f"AST pre-validation initiated for iteration {iteration}.", {"iteration": iteration})
            try:
                ...
                self.log_event("REJECT_AST_SYNTAX_ERROR", f"Candidate rejected due to AST syntax error on iteration {iteration}.", {
                    "iteration": iteration,
                    "error_message": normalized_error_msg
                })
                ...
                if version_idx > 0 and not verify_success:
                    self.log_event("ERROR", f"Unrecoverable verification failure on rollback for iteration {iteration}.")
                    self._finalize_and_generate_report()
                    return False
            ...
            self.log_event("TESTS_EXECUTED", f"Unit tests executed for iteration {iteration}. Passed: {test_result['success']}", {
                "iteration": iteration,
            ...
            if test_result["success"]:
                try:
                    ...
                    self.log_event("BENCHMARK_EVALUATED", f"Quantitative benchmark metrics evaluated for iteration {iteration}.", {
                        "iteration": iteration,
                        ...
                        "ast_valid": candidate_metrics.ast_valid
                    })
                    degraded_event, degradation_reasons = self.evaluate_performance_degradation(candidate_metrics)
                except Exception as e:
                    self.log_event("ERROR", f"Failed to evaluate candidate benchmark metrics in iteration {iteration}: {str(e)}")

            if test_result["success"] and degraded_event is None:
                # Acceptance
                ...
                if candidate_metrics:
                    ...
                    self.stable_baseline_metrics = candidate_metrics
                ...
                self.log_event("ACCEPT_NEW_BASELINE", f"Candidate accepted as new baseline version {version_idx}.", {
                    "iteration": iteration,
                    "version_idx": version_idx,
                    "metrics": {
                        "pass_rate": candidate_metrics.pass_rate if candidate_metrics else 100.0,
                        "execution_time_sec": candidate_metrics.execution_time_sec if candidate_metrics else 0.0,
                        "peak_memory_mb": candidate_metrics.peak_memory_mb if candidate_metrics else 0.0,
                        "accuracy_score": candidate_metrics.accuracy_score if candidate_metrics else 1.0
                    }
                })
                ...
                if degraded_event is not None:
                    error_msg = f"{degraded_event}: " + "; ".join(degradation_reasons)
                    self.perturbation_feedback = error_msg
                else:
                    error_msg = test_result.get("stderr", "") or test_result.get("stdout", "")
                    self.log_event("REJECT_TEST_FAILURE", f"Iteration {iteration} candidate rejected due to unit test failure.", {
                        "iteration": iteration,
                        "stderr": error_msg
                    })
                ...
                event_type = degraded_event if degraded_event is not None else "ROLLBACK"
                log_msg = f"Iteration {iteration} failed performance check ({'; '.join(degradation_reasons)}). Rolled back to stable version {version_idx}." if degraded_event is not None else f"Iteration {iteration} failed unit tests. Rolled back to stable version {version_idx}."
