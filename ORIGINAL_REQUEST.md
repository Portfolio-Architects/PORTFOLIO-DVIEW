...
## 2026-09-20T08:09:18Z

동탄 신도시 아파트 실거래 대시보드(아파트 랩 메인 홈 및 탐색 페이지)의 로딩 속도 저하 및 인터랙션 UX 지연 원인을 정밀 프로파일링하여 전면 개선하고, 초기 진입 속도(FCP/LCP) 단축 및 인터랙션 반응성(INP/TBT)을 극대화합니다.
...
### R1. 초기 진입 데이터 로딩 및 SSR/캐싱 최적화
서버 및 클라이언트 초기 로딩 과정에서 발생하는 중복 JSON I/O 및 대용량 데이터 파싱 병목을 제거하고, 불필요한 직렬 블로킹을 병렬/인메모리 캐싱으로 전환하여 첫 화면 로딩 시간(FCP/LCP)을 대폭 단축합니다.

### R2. 다중 차트 뷰포트 기반 지연 마운트(Lazy Mount) 및 메인 스레드 분산
메인 화면 상단 히어로 차트(개별 단지 시세 추이, 평형별 도넛)와 하단 매크로 통계 차트 3종(시계열 추이, 평당가 랭킹, 거래량 분포) 등 5개 이상의 무거운 Recharts 인스턴스가 첫 로딩 시 메인 스레드를 일시에 점유하지 않도록, 화면 뷰포트 진입(IntersectionObserver) 또는 유휴 시간(requestIdleCallback) 기반 단계적 렌더링을 적용하여 초기 메인 스레드 블로킹(TBT)을 획기적으로 낮춥니다.

### R3. 대용량 기간 필터링 및 통계 집계 연산 최적화
1년/전체 등 대용량 데이터셋(수 메가바이트 규모) 선택 시 발생하는 메인 스레드 연산 부하를 완화하고, 필터 변경 시 불필요한 전체 재연산 방지를 위해 캐싱 및 연산 세분화를 고도화하여 300ms 이내의 매끄러운 반응성을 유지합니다.

### R4. 제로 코스트 및 레이아웃 무결성 보존
사용자가 지정한 상단 2열 히어로 레이아웃(좌측: 평형 수요 도넛+소형 KPI 4개, 우측: 개별 단지 시세 추이) 및 하단 통계/애드센스 배치를 100% 온전히 유지하며, 불필요한 클라우드(Firestore) 직접 호출 비용 발생 없이 정적 데이터 아키텍처를 유지합니다.

## Acceptance Criteria

### 초기 로딩 및 렌더링 성능
- [ ] 메인 홈(`/`) 첫 진입 시 다중 차트 동시 마운트로 인한 메인 스레드 정체 현상 없이 화면 스켈레톤에서 핵심 콘텐츠로 매끄럽게 전환된다.
- [ ] 첫 화면 뷰포트 바깥의 차트 컴포넌트는 사용자가 스크롤하여 뷰포트에 접근할 때 점진적으로 마운트되어 초기 번들 실행 및 렌더링 부하가 분산된다.

### 인터랙션 반응성 및 연산 효율
- [ ] 권역, 기간, 평형 필터 조작 시 UI가 멈칫(Freeze)거리지 않고 300ms 이내에 즉각적인 피드백과 함께 차트/지표가 갱신된다.
- [ ] 대용량 기간(1년/전체) 데이터 전환 시에도 브라우저 응답 없음 경고 없이 부드러운 전환 인터랙션이 보장된다.

### 레이아웃 및 기능 회귀 방지
- [ ] 상단 2열 히어로 레이아웃(평형 도넛/KPI 카드 + 단지 시세 추이 차트)의 형태와 디자인 완성도가 그대로 유지된다.
- [ ] 하단 매크로 통계 4대 KPI, 하이퍼로컬 인사이트 카드, 평당가 랭킹 TOP 20 및 인라인 애드센스 슬롯이 정상 작동한다.

### 코드 품질 및 테스트 통과
- [ ] `npm test`의 핵심 컴포넌트 및 통계 엔진 단위 테스트가 100% 통과한다.
- [ ] `npx tsc --noEmit` 실행 시 TypeScript 컴파일 에러가 0건이어야 한다.
- [ ] `npm run build` 프로덕션 빌드가 성공(Exit code 0)해야 한다.
