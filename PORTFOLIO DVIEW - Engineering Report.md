...
| 분류 | 기술 | 비고 |
|:---|:---|:---|
| 2026-06-01 | **프로덕션 환경 Firebase 로그인 불능 오류 해결을 위한 CSP connect-src 도메인 예외 추가 및 실거래가 테이블 10개씩 페이징 노출 최적화 (Resolve Production Login Failure by Enhancing CSP connect-src & Optimize Transaction Table Paging to 10 Items)** | 1) PWA Service Worker가 백그라운드에서 Google reCAPTCHA 및 AdSense 관련 정적 스크립트를 로드할 때 브라우저의 CSP `connect-src` 정책에 가로막혀 네트워크 연결이 차단되고 이로 인해 로그인 기능이 응답하지 않던 오류를 해결했습니다. 2) [proxy.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/proxy.ts) 미들웨어의 `connect-src` 지시문에 `https://www.gstatic.com`, `https://*.gstatic.com`, `https://*.adtrafficquality.google`, `https://*.firebaseapp.com` 도메인을 추가하여 reCAPTCHA Enterprise, Firebase Auth, Google AdSense 소다 구성 설정 네트워크 요청을 안전하게 허용했습니다. 3) [TransactionTable.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/apartment-modal/TransactionTable.tsx) 컴포넌트의 초기 실거래가 표시 건수 및 추가 페이지 로드 개수를 기존 30개에서 10개 단위로 조정하여 모바일 및 데스크톱 렌더링 성능을 개선하고 UI 혼잡도를 완화했습니다. 4) 정적 분석(`npm run audit` PASS)을 수행하여 TypeScript 컴파일 상태, ESLint 코드 위생 및 파이어스토어 예상 비용(₩4 KRW)에 대한 모든 자체 검증을 100% 통과했습니다. |
