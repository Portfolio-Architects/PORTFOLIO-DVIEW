...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-06-06 | **아파트 탐색 리스트 뷰포트 높이 정밀 연동 및 모니터 최적화 (Viewport-based Explore List Auto-Sizing - Phase 53)** | 1) 아파트 탐색 리스트의 높이가 데스크톱 화면에서 너무 길어지거나 하단 푸터 영역을 침범하지 않고 브라우저 뷰포트에 정밀 연동되도록 리팩토링했습니다. 2) 기존 하드코딩된 최소 높이나 사이드바 전체 스크롤 크기를 강제하던 방식 대신, 리스트 컨테이너의 절대 위치(`rect.top + window.scrollY`)를 실시간 역산하여 뷰포트 크기(`window.innerHeight`)에서 안정적으로 제외하는 유연한 높이 자동 연동 수식을 수립했습니다. 3) 이를 통해 모니터 크기가 커지거나 작아져도 그에 연동되어 푸터 바로 위(12번대 항목 라인 부근)에서 리스트 가상화 스크롤바가 안착되도록 레이아웃 정합성을 대폭 높였습니다. |
