...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-05-31 | **모바일 바텀 시트 스티키 하단바 동시 노출 및 본문 그래프 이원화 고도화 (Co-expose Mobile Sticky Bar & Decouple Chart State)** | 모바일 환경에서 바텀 시트 모달이 열렸을 때 스티키 하단바(MobileDock)가 아예 가려지는 현상과 본문 하단 그래프 영역이 중복 갱신되는 현상을 완벽하게 해결했습니다. 1) [MobileDock.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/pwa/MobileDock.tsx)의 z-index를 `z-[10000]`으로 격상하여 React Portal로 뜬 바텀 시트(`z-[9999]`) 위로 항상 상호작용 가능한 상태로 드러나게 복구했습니다. 2) 하단바 노출에 의해 모달 내부 컨텐츠가 겹치는 결함을 차단하고자 바텀 시트 내부 스크롤 패딩을 `pb-24`에서 `pb-36`으로 추가 상향했습니다. 3) [MacroDashboardClient.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/MacroDashboardClient.tsx)에 viewport 감지 훅(`isMobileViewport`)을 추가하고 본문 그래프용 변수(`mainLineData`, `mainXTicks`, `mainYTicks`)를 분리하여, 모바일 뷰포트에서는 본문 하단 그래프가 항상 매크로 대표 트렌드로 고정되고 실거래 요약 바를 숨겨 정보 중복 인지 부하를 해소하고, 개별 아파트 상세 그래프는 오직 바텀 시트에서만 전담하여 렌더링하도록 흐름을 이원화했습니다. 4) `npx tsc --noEmit` 무결성 검증을 완료했습니다. |
