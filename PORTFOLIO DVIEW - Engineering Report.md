...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-06-03 | **CSP 보안 정책 완화, AdSense data-nscript 경고 제거, adsbygoogle.push availableWidth=0 에러 방어, 및 App Check reCAPTCHA v3 공급자 정정 (Ease CSP frame-src, Remove AdSense data-nscript Warning, Defend adsbygoogle availableWidth=0 TagError, & Fix Firebase App Check reCAPTCHA v3 Provider)** | 1) AdSense 내부 무효 트래픽 감시 및 품질 평가 도메인(`https://ep2.adtrafficquality.google/`)의 차단을 해제하기 위해 최상위 미들웨어 보안 정책([proxy.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/proxy.ts))의 `frame-src` 규칙에 `https://*.adtrafficquality.google`을 정밀 추가했습니다. 2) Next.js의 `<Script>` 컴포넌트를 쓸 때 강제 주입되는 `data-nscript` 관리 속성으로 인한 AdSense head tag 경고를 제거하고자, [layout.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/app/layout.tsx) 내 광고 스크립트를 HTML 표준 `<script>` 형식으로 리팩토링했습니다. 3) 모바일 앵커 광고 등이 데스크톱 뷰에서 `display: none` 처리되어 있을 때 가용 너비 0px 조건으로 인한 `adsbygoogle` TagError 오작동을 근본 해결하기 위해, [AdSense.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/ui/AdSense.tsx) 내 렌더링 검사 로직에 `getBoundingClientRect().width > 0` 및 `getComputedStyle().display !== 'none'` 등의 정밀 가시성 가드를 이식했습니다. 4) 등록된 일반 reCAPTCHA v3 사이트 키와 코드 내 공급자 스펙이 충돌하여 Firebase App Check 인증이 400 Bad Request로 누락되던 현상을 해결하기 위해, [firebaseConfig.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/lib/firebaseConfig.ts)를 `ReCaptchaV3Provider`를 올바르게 사용하도록 이관하여 클라이언트 인증 무결성을 확보했습니다. 5) `npm run build` 정적 분석 및 TypeScript 린트/컴파일 검사를 100% 통과했습니다. |
