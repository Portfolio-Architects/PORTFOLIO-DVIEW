...
| 분류 | 기술 | 비고 |
|:---|:---|:---|
| 2026-06-07 | **동탄 시장 요약 종합 모니터링 위젯 신설 및 레이아웃 대칭화 (Phase 112)** | 1) 갭투자 Top 5 카드가 좌측에 마운트되면서 발생한 우측 하단의 공백을 메우기 위해 기존 차트 패널 내부에 임시 분기로 존재하던 '동탄 시장 요약' 뷰를 완전히 외부로 분리했습니다. 2) 우측 차트 카드와 1:1 대칭을 이루는 독립형 '종합 모니터링' 카드를 신설하고, 우측 열을 감싸는 `Right Column Container`를 설계하여 두 카드가 수직 정렬되도록 레이아웃을 개편했습니다. 3) 2x2 그리드 내에 행정동 필터(`gapRankingDong`)와 양방향 연계되는 '동탄 평균 전세가율', '시장 평균 매매/전세가', '최고 관심 단지' 연동 모달 핸들러를 탑재했습니다. 4) 하단 마켓 시그널 영역에 최근 7일 거래량 추이 배지 및 거래 활성도 신호등(🟢/🟡/🔴) 팩트시트를 이식했습니다. 5) 데스크톱 뷰포트에서 좌측 `dailyTimeline` (420px) + `3 Info Boxes` (112px) = 548px 높이와 수평을 맞추기 위해 우측 차트 카드의 최소 높이를 `md:min-h-[548px]` 로 늘리고 내부 차트 높이를 `md:h-[370px]` 로 확장 튜닝했으며, 좌우 하단 카드(소액 갭투자 Top 5, 종합 모니터링)에 모두 `md:min-h-[360px]` 최소 높이를 적용하여 대시보드 하단의 흰 여백(White space) 불균형을 완전히 제거했습니다. 6) `npx tsc --noEmit` 컴파일 및 Jest 회귀 테스트(18 suites / 94 tests)를 100% 무결하게 통과했습니다. |
