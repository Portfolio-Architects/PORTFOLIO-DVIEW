...
| 2026-05-31 | **일자별 신고가 타임라인 모바일 아파트명 텍스트 잘림 해결 (Fix apartment name truncation in daily timeline on mobile)** | 일자별 신고가 타임라인의 각 단지 아이템에서 모바일 뷰포트 너비가 좁을 때 아파트 이름이 말줄임표(...)로 잘리던 문제를 해결했습니다. 1) [동 정보 + 아파트명] 영역과 [평형 + 층수] 영역을 한 줄에 나란히 배치하던 flex 구성을 상하 2단 계층 구조로 리팩토링했습니다. 2) 아파트명 텍스트에서 `truncate` 클래스를 제거하고 `break-keep`을 적용하여 단어 단위 줄바꿈을 유도함으로써 아파트명이 아무리 길어도 끝까지 온전하게 노출되도록 개선했습니다. 3) 평형 및 층 정보는 하단 보조 라인으로 분리하여 가시성을 확보하고, 전체 빌드 무결성(`npm run audit` PASS) 및 Jest 테스트 통과(44/44 Pass)를 확인했습니다. |
