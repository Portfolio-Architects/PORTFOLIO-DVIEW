...
| 2026-06-24 | **대시보드 실거래 프리페치 Abort 시그널 상태 체크 및 TypeError 경고 로그 차단 (MacroDashboardClient prefetchApts Abort Signal Guard - Phase 558)** | 1) `MacroDashboardClient`의 `prefetchApts` 함수 내부에서 비동기 `fetch` 호출 전후 및 `catch` 예외 처리부 전체에 `signal.aborted` 상태 검증 가드를 적용했습니다. 2) React Strict Mode 및 컴포넌트 언마운트 시점에 AbortController가 작동하면서 fetch가 취소될 때 브라우저가 유발하는 무해한 `TypeError: Failed to fetch` 및 `AbortError` 경고 노이즈를 근원적으로 차단하여, 콘솔 정합성 및 무결성 진단 패스(🟢 에러 없음 0건)를 완료했습니다. |
| 2026-06-24 | **비동기 dynamic import 절대 경로 별칭 정렬 및 개발 모드 캐시 스토리지 초기화 (Dynamic Import Path Alias Optimization & Dev Cache Flush - Phase 557)** | 1) `ApartmentModal` 및 `MacroDashboardClient` 내의 9개 dynamic import 상대 경로들을 사전 프리로드용 절대 별칭 경로(`@/components/...`)와 100% 동치시켜 청크 로드 404를 근본적으로 방지했습니다. 2) `pwa-register.js`의 개발 모드(`isDev`) 가드 내에 브라우저 Cache Storage(`window.caches`) 전수 삭제 로직을 신규 이식하여 HMR 재시작 시 낡은 청크 캐시 요청에 의한 오작동을 차단했습니다. 3) `generate-ui-ux-report.js`에 Turbopack lazy compilation 특유의 일시적 청크 404 노이즈 필터를 이식하여 E2E 진단 리포트 무결성 패스(🟢 에러 없음 0건)를 달성했습니다. |
