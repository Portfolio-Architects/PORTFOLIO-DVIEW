...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-06-02 | **Firebase 로그인 지연 해결을 위한 App Check 리프레시 키 매핑 보정 및 Next.js 16 Proxy 사양 충돌 조치 (Resolve Production Login Delay via App Check Key Fallback & Fix Next.js 16 Proxy Conflict)** | 1) Firebase App Check를 `ReCaptchaEnterpriseProvider`로 초기화할 때 Vercel 환경에 등록된 `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` 변수를 감지 및 폴백으로 연동하여, 잘못된 키 로딩으로 인한 Firestore 요청 무한 대기(2분 이상 지연) 현상을 완벽히 해결했습니다. 2) Next.js 16 버전에서 도입된 `proxy.ts` 표준 사양과 중복 충돌을 일으키던 구식 `middleware.ts` 엔트리를 제거하여, Vercel 빌드 타임 에러(`Both middleware file and proxy file are detected`)를 완벽히 해결하고 빌드 무결성을 확보했습니다. 3) `npm run audit` 정적 컴파일 및 린트 검사를 100% 통과(SUCCESS)했습니다. |
