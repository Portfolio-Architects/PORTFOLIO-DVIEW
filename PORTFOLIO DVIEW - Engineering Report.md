...
> **Date**: 2026-06-02 | **Grade**: A+ | **Branch**: master | **Status**: Active Development & Stabilization
...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-06-03 | **Vercel Hobby 플랜 크론 스케줄 제한 우회 (Workaround Vercel Hobby Plan Cron Limit)** | Vercel Hobby 플랜 계정의 1일 1회 실행 제한 규정으로 인해 배치 수집 크론(`/api/cron/sync-local-notices`)의 하루 2회(0 3,15 * * *) 스케줄 설정 시 배포가 실패하는 문제를 포착하여, 하루 1회 실행 스케줄(`0 3 * * *`)로 완화 및 재조정했습니다. |
| 2026-06-03 | **행정망 자동 수집 크론 스케줄 추가 (Add Cron Schedule for Local Notices)** | 동탄구 소식 행정망 자동 수집 파이프라인의 실행 주기를 사용자의 요구에 맞춰 한국 시간(KST) 오전/오후 12시(00:00, 12:00)로 설정했습니다. Vercel Cron의 UTC 기준 작동 특성에 맞추어 `vercel.json`에 `/api/cron/sync-local-notices` 경로의 크론 스케줄을 `0 3,15 * * *` (UTC 3시, 15시)로 등록했습니다. |
| 2026-06-03 | **CSP 보안 정책 완화, AdSense data-nscript 경고 제거, adsbygoogle.push availableWidth=0 에러 방어, 및 App Check reCAPTCHA v3 공급자 정정 (Ease CSP, Remove AdSense data-nscript Warning, Defend adsbygoogle availableWidth=0 TagError, & Fix Firebase App Check reCAPTCHA v3 Provider)** | 1) AdSense 내부 무효 트래픽 감시 및 품질 평가 도메인(`*.adtrafficquality.google`)의 차단을 해제하기 위해 최상위 미들웨어 보안 정책([proxy.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/proxy.ts))의 `frame-src` 및 `img-src` 규칙에 `https://*.adtrafficquality.google`을 추가했습니다. 2) Next.js의 `<Script>` 컴포넌트를 쓸 때 강제 주입되는 `data-nscript` 관리 속성으로 인한 AdSense head tag 경고를 제거하고자, [layout.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/app/layout.tsx) 내 광고 스크립트를 HTML 표준 `<script>` 형식으로 리팩토링했습니다. 3) 모바일 앵커 광고 등이 데스크톱 뷰에서 `display: none` 처리되어 있을 때 가용 너비 0px 조건으로 인한 `adsbygoogle` TagError 오작동을 근본 해결하기 위해, [AdSense.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/ui/AdSense.tsx) 내 렌더링 검사 로직에 `getBoundingClientRect().width > 0` 및 `getComputedStyle().display !== 'none'` 등의 정밀 가시성 가드를 이식했습니다. 4) 등록된 일반 reCAPTCHA v3 사이트 키와 코드 내 공급자 스펙이 충돌하여 Firebase App Check 인증이 400 Bad Request로 누락되던 현상을 해결하기 위해, [firebaseConfig.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/lib/firebaseConfig.ts)를 `ReCaptchaV3Provider`를 올바르게 사용하도록 이관하여 클라이언트 인증 무결성을 확보했습니다. 5) 아파트 상세 페이지(Server Component) 내 JSON-LD 스키마 인라인 스크립트 `<script type="application/ld+json">` 로딩 시 nonce 누락으로 인한 CSP 차단 문제를 잡고자, [page.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/app/apartment/[aptName]/page.tsx)에서 `headers()`의 `x-nonce`를 읽어와 태그에 직접 nonce를 주입하도록 조치했습니다. 6) 로컬 개발 환경(`development`)에서 Next.js Turbopack HMR(핫 리로드) 스크립트 실행이 CSP에 의해 차단되던 현상을 예방하기 위해, 개발 서버 모드 진입 시 미들웨어 CSP 주입을 바이패스(Bypass)하도록 분기 처리했습니다. 7) `npm run build` 정적 분석 및 TypeScript 린트/컴파일 검사를 100% 통과했습니다. |
| 2026-06-02 | **Firebase 로그인 지연 해결, Next.js 16 Proxy 사양 충돌 조치, 및 구글 애드센스 스크립트 전역 객체 바인딩 오류 수정 (Resolve Production Login Delay, Next.js 16 Proxy Conflict, & Fix Google AdSense Global Scope Binding Bug)** | 1) Firebase App Check를 `ReCaptchaEnterpriseProvider`로 초기화할 때 Vercel 환경에 등록된 `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` 변수를 감지 및 폴백으로 연동하여, 잘못된 키 로딩으로 인한 Firestore 요청 무한 대기(2분 이상 지연) 현상을 완벽히 해결했습니다. 2) Next.js 16 버전에서 도입된 `proxy.ts` 표준 사양과 중복 충돌을 일으키던 구식 `middleware.ts` 엔트리를 제거하여, Vercel 빌드 타임 에러(`Both middleware file and proxy file are detected`)를 완벽히 해결하고 빌드 무결성을 확보했습니다. 3) AdSense 컴포넌트 내에서 `adsbygoogle.push()`가 전역 `window` 객체에 정상적으로 등록되지 못해 광고 렌더링이 무반응하는 스코프 누수 버그를 수정했습니다. 4) `npm run audit` 정적 컴파일 및 린트 검사를 100% 통과(SUCCESS)했습니다. |
| 2026-06-02 | **PWA 서비스 워커 및 동적 JSON 데이터 캐싱 문제 해결을 통한 하드 리프레시 없는 최신 정보 자동 반영 구조 확보 (Resolve stale content caching by auto-bumping Service Worker cache version & enforcing Cache-Control headers)** | 1) 사용자가 일반 새로고침이나 모바일 PWA 앱 재진입 시 최신 실거래 정보가 표시되지 않고 하드 리프레시를 해야만 최신화되는 중대한 문제를 분석 및 해결하여 DAU 활성화 병목을 제거했습니다. 2) [next.config.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/next.config.ts)에 `headers()` 설정을 도입하여 `/sw.js`, `/manifest.webmanifest`, `/data/` 및 `/tx-data/` 하위의 모든 JSON 파일들에 `Cache-Control: no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0` 헤더를 강제 주입해 브라우저/CDN의 오작동 및 캐싱 정체를 방지했습니다. 3) [update-sw-version.js](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/scripts/update-sw-version.js) 스크립트를 신설하고 [package.json](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/package.json)의 `build` 프로세스에 이식하여, 빌드 타임마다 서비스 워커 파일 내 `CACHE_NAME`과 `DYNAMIC_CACHE_NAME`을 빌드 타임스탬프(`v-Date.now()`) 기반으로 자동 갱신해 브라우저가 새 서비스 워커 설치 및 이전 캐시 제거를 강제 유도하도록 파이프라인을 구축했습니다. 4) 로컬 프로덕션 빌드 및 로컬 서버 검증을 통해 응답 헤더 추가 및 서비스 워커 내 버전 치환이 정상 동작하는 것을 전수 검증했습니다. |
| 2026-06-02 | **구글 애드센스 로드 및 리소스 표시를 차단하는 CORP 보안 헤더 설정 완화 (Resolve Google AdSense Rendering Block via Easing CORP Security Header to cross-origin)** | 1) 라이브 프로덕션 서버에서 구글 애드센스 스크립트와 배치 광고 박스는 정상 렌더링되나 광고 콘텐츠(이미지, 동적 미디어 등)가 실제 표출되지 않고 빈 박스로 차단되던 심각한 런타임 오류를 해결했습니다. 2) 실시간 웹 응답 헤더 수집 검증 결과, 최상위 미들웨어 보안 정책([proxy.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/proxy.ts))에서  Spectre 및 메모리 탈취 방지를 위해 탑재한 `Cross-Origin-Resource-Policy` (CORP) 헤더가 `'same-origin'`으로 과도하게 하드닝되어 있어, 브라우저가 타사 광고 CDN 리소스들의 메모리 로딩을 강제로 중단시켰던 병목을 잡아냈습니다. 3) 해당 CORP 헤더 설정을 `'cross-origin'`으로 완화하여, 구글 및 더블클릭 광고 네트워크의 자원들이 브라우저 보안 장벽에 걸리지 않고 화면에 완벽하게 렌더링될 수 있도록 보정했습니다. 4) `npx tsc --noEmit`을 돌려 컴파일 정합성을 검증했습니다. |
| 2026-06-01 | **첫 로그인 시 신규 가입자 닉네임 설정 강제 오버레이 모달 적용 및 기본 닉네임 수정 (Enforce Nickname Creation for New Users & Update Default Nickname)** | 1) 구글/기타 로그인으로 첫 가입한 신규 유저에게 닉네임 설정을 강제하는 전체 화면 블로킹(오버레이) 모달을 도입하고, 닉네임 설정 전에는 전체 대시보드 페이지 탐색을 원천 블로킹하도록 변경했습니다. 2) 신규 유저 가입 시 생성되는 기본 닉네임이 기존 관리자 닉네임과 겹치던 문제를 해결하기 위해 `DEFAULT_NICKNAME`을 `'매니저'`에서 `'임시_임장러'`로 변경하고, `isValidNickname` 검사 시 공백 제거 및 특수문자 방지 정규식 검사를 강화했습니다. 3) 기존 유저(예: '매니저' 등)는 닉네임 변경 대상에서 완전히 배제하여 하위 호환성을 유지할 수 있도록 `UserProfile` 스키마 및 DB에 `hasSetNickname?: boolean` 필드를 신설했습니다. 신규 가입자는 `false`로 생성되고 닉네임 설정 완료 시 `true`로 업데이트됩니다. 4) 닉네임 입력 모달은 `backdrop-blur-md` 기반의 토스(Toss) 스타일 다이얼로그로 구현하였으며, `npx tsc --noEmit`을 통해 정적 컴파일 및 무결성을 완전 검증했습니다. |
| 2026-06-01 | **프로덕션 로그인 상태 갱신 병렬화 적용 및 네트워크 Waterfall 성능 병목 해결 (Optimize Production Login Performance via Parallel Session & Profile Loading)** | 1) 구글 OAuth 로그인 완료 후, 클라이언트 인증 상태가 활성화(로그인 처리 및 탭 갱신)되는 응답 속도가 현저히 느리던 결함을 해소했습니다. 2) [useAuth.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/hooks/useAuth.ts)의 `onAuthStateChanged` 이벤트 핸들러 내에서 Firestore 프로필 로드, 사용자 권한 확인, 구매 내역 연동, 그리고 서버 HttpOnly 쿠키 발급 API(`/api/auth/session`) 호출 작업이 각각 `await` 지시문에 의해 연속적인 직렬 폭포수(Waterfall, RTT 4회)로 수행되던 병목을 포착했습니다. 3) `Promise.all` 및 프로미스 체이닝을 활용해 토큰 발급/쿠키 발급 프로세스와 Firestore 사용자 프로필/구매 내역 fetch 프로세스를 완전 병렬(Parallel)로 구성하여 네트워크 RTT를 1회 수준으로 단축하고, 로그인 전환 지연을 즉시 해결했습니다. 4) `npx tsc --noEmit` 타입 검사를 수행해 무결성을 확보했습니다. |
| 2026-06-01 | **철도교통 KPI 카드 클릭 시 소식 내 '교통-철도' 카테고리 탭 즉시 활성화 및 해시 라우팅 연동 (Deep-link Transit Notice KPI Card to Lounge Transit Filter Tab)** | 1) 메인 대시보드의 '동탄 철도교통 소식' 카드 클릭 시 개별 공지 팝업을 직접 띄우던 기존 흐름을 개편하여, 전체 철도교통 소식 목록을 한눈에 볼 수 있도록 커뮤니티 라운지 내 **'동탄구 소식' ➔ '교통-철도' 탭**으로 직접 유도했습니다. 2) [MacroDashboardClient.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/MacroDashboardClient.tsx) 클릭 핸들러의 대상을 `#lounge-notices-rail` 해시로 변경했습니다. 3) [LoungeFeedClient.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/LoungeFeedClient.tsx)의 `checkParams` 및 해시 분석 함수에 `#lounge-notices-rail` 조건식을 추가해 해당 진입 시 `activeSubCategory`를 `'rail'`로 강제 전환하게 설계했습니다. 4) 기존 `checkParams` 내의 `removeEventListener` cleanup 타겟 오타(`'removeEventListener'` ➔ `'hashchange'`) 버그를 함께 핫픽스하고 무결성을 확인했습니다. |
| 2026-06-01 | **대시보드 KPI 카드 중 로컬 소식 주제를 동탄 철도교통으로 한정 및 커스텀 필터링 적용 (Restrict Local Notice KPI Card Topic to Dongtan Rail/Transit)** | 1) 메인 대시보드의 '오늘의 주요 로컬 소식' KPI 카드의 범위를 동탄 주민들의 최대 관심사인 철도교통 소식으로 좁히기 위해, 타이틀을 '동탄 철도교통 소식'으로 변경하고 툴팁 안내를 그에 맞춰 보강했습니다. 2) [MacroDashboardClient.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/MacroDashboardClient.tsx)에 `railNotices` 필터링 훅을 신설하여, 소식 데이터 중 `source === 'rail'` 이거나 제목에 철도교통 관련 핵심 키워드(철도, 교통, GTX, 트램, 인동선, SRT, 지하철 등)가 포함된 경우만 추려 렌더링되도록 제한했습니다. 3) `npx tsc --noEmit` 정적 타입 검사를 실행하여 빌드 무결성을 최종 검증했습니다. |
| 2026-06-01 | **실거래/시장 요약 카드 텍스트 크기 상향 조정 및 가독성 개선 (Slightly Increase Text Sizes in Real-estate/Market Summary Cards for Readability)** | 1) 모바일 및 데스크톱 환경에서 아파트 상세 실거래 요약 바와 동탄 시장 요약 카드의 데이터 가시성을 개선하기 위해 텍스트 크기를 상향했습니다: 2) [MacroDashboardClient.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/MacroDashboardClient.tsx)의 3열 실거래 그리드 지표들의 라벨 크기를 기존 `text-[10px]/text-[10.5px]`에서 `text-[11px]/text-[12px]`로, 매매/전세/갭투자 수치(금액) 텍스트 크기를 기존 `text-[12px]/text-[12.5px]/text-[13.5px]`에서 `text-[13.5px]/text-[14px]/text-[15.5px]`로, 예상 갭 투자율(%) 텍스트를 `text-[9.5px]/text-[9px]`에서 `text-[10.5px]`로 각각 상향 조정했습니다. 3) `npm run audit` 정적 분석 및 TypeScript 린트 무결성을 최종 검증했습니다. |
| 2026-06-01 | **Firebase 로그인 CSP 예외 추가, 실거래가 테이블 10개씩 페이징 최적화, 및 공시지가 현실화율 관련 기능 제거 (Resolve Firebase Login CSP, Optimize Transaction Paging & Remove Public Price Realization Functionality)** | 1) reCAPTCHA 및 Google AdSense 관련 PWA CSP `connect-src`에 `https://www.gstatic.com` 등 도메인 예외를 추가해 로그인 무반응 오류를 해결했습니다. 2) [TransactionTable.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/apartment-modal/TransactionTable.tsx)의 페이지당 실거래가 로드 단위(30건 ➔ 10건)를 최적화하여 렌더링 오버헤드를 해소했습니다. 3) 유저 피드백에 기반하여 아파트 상세 모달 내 공시가격 연동 자산 안전성 분석 바, 차트 내 공시가격 점선 그래프, 툴팁 정보 및 범례를 포함한 공시지가 현실화율 관련 로직 및 UI 컴포넌트들을 [TransactionChartSection.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/apartment-modal/TransactionChartSection.tsx)에서 전면 소거했습니다. 4) 정적 분석(`npm run audit` PASS) 무결성을 검증했습니다. |
| 2026-05-31 | **구글 OAuth 인앱 브라우저 'disallowed_useragent' 403 오류 해결을 위한 사용자 제스처 기반 외부 브라우저(크롬/사파리) 우회 이동 레이어 고도화 (Bypass Google Sign-In 403 disallowed_useragent via Gesture-driven External Browser Bypass)** | 구글 보안 정책상 네이버, 카카오톡, 라인, 인스타그램 등 인앱 브라우저(WebView) 환경에서 구글 OAuth 로그인이 차단(`403 disallowed_useragent` 에러)되는 문제를 해결하기 위해, 사용자 제스처 기반 외부 브라우저(안드로이드: Chrome, iOS: Safari) 강제 이동 우회 레이어를 대대적으로 개선했습니다. 1) [InAppBrowserBypass.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/pwa/InAppBrowserBypass.tsx) 컴포넌트가 모든 인기 인앱 브라우저(KakaoTalk, NAVER app, LINE, Instagram, Facebook, X/Twitter 등) 감지 시 Toss-Style of Premium 경고 모달을 띄우도록 변경했습니다. 2) 기존 page load 시점의 자동 `location.href` 리다이렉트가 모바일 앱의 보안 정책으로 인해 차단될 수 있는 한계를 극복하고자, 명시적인 사용자 클릭(User Gesture)으로 작동하는 '크롬(또는 사파리)으로 열기' 버튼을 제공하여 100% 신뢰성 있는 외부 브라우저 이동을 실현했습니다. 3) iOS 타사 인앱(인스타그램 등) 사용자를 위해 1-Click 주소 복사(fallback text-area 복사 헬퍼 내장) 기능 및 사파리 수동 이동 가이드를 제공하고, 구글 로그인 외 단순 정보 검색만 원하는 사용자의 이탈을 방지하고자 모달 닫기('그냥 둘러볼래요') 버튼을 함께 안착시켰습니다. 4) 정적 분석(`npm run audit` PASS)을 통해 무결성을 최종 검증했습니다. |
...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-06-01 | **프로덕션 환경 Firebase 로그인 불능 오류 해결을 위한 CSP connect-src 도메인 예외 추가, 실거래가 테이블 10개씩 페이징 최적화, 및 공시가 현실화율 바 텍스트 개행 문제 개선 (Resolve Production Login Failure, Optimize Transaction Table Paging to 10 Items & Fix Public Price Realization Bar Layout)** | 1) PWA Service Worker가 백그라운드에서 Google reCAPTCHA 및 AdSense 관련 정적 스크립트를 로드할 때 브라우저의 CSP `connect-src` 정책에 가로막혀 네트워크 연결이 차단되고 이로 인해 로그인 기능이 응답하지 않던 오류를 해결했습니다. 2) [proxy.ts](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/proxy.ts) 미들웨어의 `connect-src` 지시문에 `https://www.gstatic.com`, `https://*.gstatic.com`, `https://*.adtrafficquality.google`, `https://*.firebaseapp.com` 도메인을 추가하여 reCAPTCHA Enterprise, Firebase Auth, Google AdSense 소다 구성 설정 네트워크 요청을 안전하게 허용했습니다. 3) [TransactionTable.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/apartment-modal/TransactionTable.tsx) 컴포넌트의 초기 실거래가 표시 건수 및 추가 페이지 로드 개수를 기존 30개에서 10개 단위로 조정하여 모바일 및 데스크톱 렌더링 성능을 개선하고 UI 혼잡도를 완화했습니다. 4) [TransactionChartSection.tsx](file:///c:/Users/ocs56/OneDrive/바탕 화면/PORTFOLIO/PORTFOLIO - DVIEW/frontend/src/components/apartment-modal/TransactionChartSection.tsx) 컴포넌트 내 공시가 현실화율 분석 바의 텍스트가 모바일 환경에서 강제 줄바꿈(wrapping)되어 깨지던 오작동을 해결하기 위해 모바일에서 세로로 정렬되는 flex-col 레이아웃을 도입하고 whitespace-nowrap 지시문을 매칭하여 시인성을 제고했습니다. 5) 정적 분석(`npm run audit` PASS)을 수행하여 TypeScript 컴파일 상태, ESLint 코드 위생 및 파이어스토어 예상 비용(₩4 KRW)에 대한 모든 자체 검증을 100% 통과했습니다. |
