...
| 일시 | 주요 항목 | 요약 내용 |
|:---|:---|:---|
| 2026-06-06 | **아파트 탐색 리스트 최소 높이 확보 및 사이드바 정합 버그 수정 (Ensure Minimum Explore List Height Alignment - Phase 53)** | 1) 데스크톱 해상도에서 화면 높이가 작거나 사이드바가 길어질 경우, 가상화 리스트(`listHeight`)가 작게 잡혀 왼쪽 법정동 사이드바의 마지막 항목인 '석우동' 라인에 미치지 못하고 하단에 넓은 빈 공백이 생기던 레이아웃 불일치 문제를 해결했습니다. 2) 데스크톱 환경에서는 사이드바 엘리먼트(`sidebarRef`)의 실제 전체 높이(`scrollHeight`)를 동적으로 측정하여, 최소한 사이드바 영역 전체(석우동 라인 바닥)가 다 보일 수 있는 높이를 가상화 리스트의 최소 높이로 삼도록 개선했습니다. 3) 이를 통해 브라우저 높이에 관계없이 아파트 탐색 리스트의 바닥이 사이드바와 딱 맞게 연장되도록 보장했습니다. |
